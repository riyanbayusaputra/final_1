<div class="max-w-[480px] mx-auto bg-white min-h-screen relative pb-20">
    <!-- Header -->
                ...
                <div>
                    ...
                    <div class="px-4 py-3 flex gap-3">
                        @if($item->product->first_image_url)
                            <img src="{{$item->product->first_image_url}}" alt="{{$item->product->name}}" class="w-12 h-12 object-cover rounded-lg" loading="lazy">
                        @else
                            <svg class="w-12 h-12 text-gray-200 rounded-lg" fill="currentColor" viewBox="0 0 24 24"><rect width="24" height="24" rx="6"/></svg>
                        @endif
                        <div>
        ...
        <div class="mt-4 bg-white p-4 flex justify-center">
            <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center space-x-2">
                ...
                @php
                    ...
                    if ($start < 1) { $start = 1; $end = 2; }
                    if ($end > $orders->lastPage()) { $start = $orders->lastPage() - 1; $end = $orders->lastPage(); }
                @endphp
